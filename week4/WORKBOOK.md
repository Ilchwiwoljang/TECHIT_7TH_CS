...
SQL(관계형 데이터베이스)와 NoSQL(비관계형 데이터베이스)은 데이터를 저장, 조회, 관리하는 방법이 다릅니다.<br> 면접관은 후보자가 이 두 시스템의 차이점을 이해하고 있는지 확인하고 싶어할 것입니다. <br><br>
...
특징으로는 **데이터를 2차원 테이블로 표현**하며, 서로 다른 **테이블들이 관계를 맺고 있다**는 것입니다.<br> 또한, RDB는 **스키마를 통해 데이터 구조를 엄격하게 정의**합니다. <br><br>
...
---
## 3-1 NoSQL 특징
- NoSQL(Not Only SQL)은 쉽게 확장되는 유연한 스키마에 데이터를 저장하는 데이터베이스 기술입니다.


### 💡 NoSQL 데이터베이스의 종류는 크게 네 가지로 나눌 수 있습니다.<br>
...
Java의 HashMap 자료구조와 같이 Key 하나에 Value 데이터가 전달되는 구조이며,<br> 이 구조는 특히 **빠른 검색 과 데이터 접근을 위해 최적화되어 있습니다**. 대표적으로 `Redis`가 있습니다. <br><br>
...
### 📂 Document 구조
**데이터를 테이블이 아닌 문서처럼 저장하는 형태**이며, **JSON 형태의 데이터를 BSON(Binary JSON)으로 문서화 하여 저장**하며,<br> 컬렉션이라고 하는 그룹으로 묶어서 관리됩니다. 대표적으로는 `MongoDB`가 있습니다. <br><br>

- #### **MongoDB 특징**
  - **문서 지향** 데이터베이스 입니다.
  - **유연한 스키마**를 가지며, 이로 인해 요구 사항이 변화함에 따라 데이터베이스 스키마를 쉽게 조정할 수 있습니다. <br><br>
  <img src="img/mongodb1.png" width="400"><br><br><br><br>
...
**수평적 확장성(Scale Out): NoSQL**
  ...
  - **확장에 용이**해서 대용량 데이터 처리와 높은 트랙픽을 관리하는데 적합합니다. <br><br>
...
- 데이터간 관계를 맺고 있으며, 자주 업데이트 되는 경우
- JOIN 연산과 INDEX 관리가 중요한 경우

**NoSQL**
- 대용량 데이터를 다루고 높은 확장성이 요구되는 경우
- 데이터 읽기 작업은 빈번하지만, 자주 업데이트 되지 않는 경우
...
반면, NoSQL은 스키마가 없어 데이터 구조의 자유로운 변경과 수평적 확장성을 가능하게 하지만,<br> 데이터 중복이 발생하거나 데이터 변경 시 연산이 지연될 수 있습니다.
...
반면에, NoSQL은 대용량 데이터 처리와 높은 확장성이 중요한 환경에 적합하며<br> 데이터 읽기 작업이 빈번하나 업데이트는 상대적으로 적게 발생하는 경우에 유리합니다.
